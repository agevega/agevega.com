...
resource "aws_cloudwatch_log_delivery_source" "cf_landing" {
  ...
  name         = "${var.project_name}-bastion-cf-landing"
  log_type     = "ACCESS_LOGS"
  resource_arn = aws_cloudfront_distribution.bastion_distribution_landing.arn
...
resource "aws_cloudwatch_log_delivery_destination" "cf_access_logs" {
  ...
  name = "${var.project_name}-bastion-cf-access-logs"
...
resource "aws_cloudwatch_log_delivery_source" "cf_academy" {
  ...
  name         = "${var.project_name}-bastion-cf-academy"
  log_type     = "ACCESS_LOGS"
  resource_arn = aws_cloudfront_distribution.bastion_distribution_academy.arn
