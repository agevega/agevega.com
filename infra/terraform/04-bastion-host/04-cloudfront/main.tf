...
# ------------------------------------------------------------------------------
# Academy CloudFront distribution (origin: same EC2 bastion, port 8443)
#
...
resource "aws_cloudfront_distribution" "bastion_distribution_academy" {
  enabled         = true
  is_ipv6_enabled = true
  comment         = "Bastion Host Origin (Module 04) - Distribution for dev.academy.${var.domain_name}"
...
resource "aws_ssm_parameter" "cloudfront_distribution_id_academy" {
  name        = "/${var.project_name}/04-bastion-host/04-cloudfront/cloudfront-distribution-id-academy"
  description = "CloudFront Distribution ID (Bastion academy)"
  type        = "String"
